 ...
 * @author     Andropov Vladimir
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL
 * @copyright  (C) 2013-2014 Andropov Vladimir <vvvvvvvvvv@inbox.ru>
 *
 */
			...
			if (isset($mode) && $mode == 'range') {
				...
				foreach ($results as $result) {
					...
					foreach ($ranges as $range) {
					 ...
					 // this is web http
						$webcourse = $range->getAttribute('webcourse');
						if (empty($webcourse)) {
							$webcourse = 'Http://tax.elpronet.ru'; // some weblink
						}
						$range_array = array_merge($range_array, array(
							$low . "|" . $high . "|" . $include . "|" . $description . "|" . $webcourse
						));
					}
				...
				foreach ($results_summary as $key => $value) {
					...
					foreach ($results as $result) {
						list($low, $high, $include, $description, $webcourse) = explode('|', $result);
						switch ($include) {
							case 'none':
								if ($low < $condition && $condition < $high) {
									...
									$summary_html .= ' <a href="';
                                                                         $summary_html .= $webcourse;
                                                                         $summary_html .= '    ">Пройти курс</a>';
